...
export default class MathUtils {
	...
	static clampAuto(value:number, clamp1:number = 0, clamp2:number = 1):number {
		if (clamp2 < clamp1) {
			var v:number = clamp2;
			clamp2 = clamp1;
			clamp1 = v;
	 ...
	 */
	static map(value:number, oldMin:number, oldMax:number, newMin:number = 0, newMax:number = 1, clamp:Boolean = false):number {
	 ...
	 */
	// Need a better name?
	static rangeMod(value:number, min:number, pseudoMax:number):number {
	...
	static getHighestPowerOfTwo(value:number):number {
